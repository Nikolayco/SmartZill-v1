...
# TEST WITH HLS URL (M3U8) which is harder to stream
RADIO_URL = "https://moondigitaledge.radyotvonline.net/slowtime/playlist.m3u8"
PORT = 5959

print(f"Initializing VLC Instance for HLS Radio Stream Test...")
try:
...
# Configure Streaming Options (Same as production but with HLS suspect)
# TRYING improved config: acodec, audio-sync
...
print(f"Streaming HLS URL: {RADIO_URL}")
print(f"Streaming Option: :sout={sout}")

media.add_option(f":sout={sout}")
media.add_option(":sout-keep")
media.add_option(":network-caching=2000") # More caching for HLS
